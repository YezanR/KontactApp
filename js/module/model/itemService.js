(function(){
  ...
  function __ItemService($http, $rootScope, $filter, Utils)
  {
    ...
    */
    ...
    {
      console.log("Service deleting item ...");
      var _type;
      if ( item instanceof User )
      {
         _type = "user";
      ...
      console.log(DATA_LOCATION+"?type="+_type+"&action=delete&iid="+item.iid);
      $http.get(DATA_LOCATION+"?type="+_type+"&action=delete&iid="+item.iid)
        .then(function(response) {
          console.log("server response "+response.data);
          //OK
          if ( response.status = 200 )
          {
            list = $filter('filter')(list, { iid: "!" + item.iid});
    ...
    function attachUserToGroup(user, groupId)
    {
      if ( user && groupId )
      {
        for(var i=0; i<groupId.length; i++)
        {
            ...
            // add user to group as a child
            var group = getGroupById(groupId[i]);
            if ( group != undefined )
            {
              group.childrenId.push(user);
            }
        }
      }
    }
    ...
    /*============= getters functions from the Database =================*/
    ...
    /*============ Locally getters functions ( from list[] ) =============*/
    function getGroupById(groupId)
    {
      var filteredList = $filter('filter')(list, { iid: groupId });
